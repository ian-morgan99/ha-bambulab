...
Advanced options should only be used for non-standard setup or privacy requirements.

- **Disable SSL verification** - disabled certificate verification for MQTT, FTP and chamber camera connections to printer (unsafe).
- **Incognito mode** - Automatically deletes timelapse videos and model files from the printer after each print completes. This privacy feature helps keep your printer's storage clean and removes identifiable print data. Files are deleted approximately 10 minutes after print completion via FTP.
